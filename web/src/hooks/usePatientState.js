...
export function usePatientState() {
  ...
  useEffect(() => {
    ...
    const unsubRec = onValue(recQ, (snap) => {
      ...
      console.warn('medicalRecords:', err.code);
      setMedicalRecord(null); // deblochează loading chiar dacă avem eroare de permisiuni
    });
    ...
    const unsubLeg = onValue(legQ, (snap) => {
      ...
      console.warn('pacienti:', err.code);
      setLegacyPatientDoc(null);
    });
    ...
    const unsubReq = onValue(reqQ, (snap) => {
      ...
      console.warn('appointmentRequests:', err.code);
      setAppointmentRequest(null);
    });
