...
export default function FirstConsultationModal({ open, onClose, appointmentRequest }) {
  ...
  // Resetăm tot la închidere; pre-completăm la deschidere cu date din cerere
  useEffect(() => {
    if (!open) {
      setStep(0);
      setError('');
      setDevStatus('idle');
      setDeviceId('');
      setPatientData({ prenume: '', nume: '', varsta: '', telefon: '', email: '' });
      setMedicalData({ anamneza: '', alergii: '', diagnosticCurent: '' });
  ...
  const handleActivate = async () => {
    if (!appointmentRequest?.pacientUid) {
      setError('UID pacient lipsă. Reîncărcați pagina și reîncercați.');
      return;
    }
    setSaving(true);
    setError('');
